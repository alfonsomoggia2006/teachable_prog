...
def main(cartella_immagini, model_path, labels_path):
    ...
    for filename in os.listdir(cartella_immagini):
        if filename.endswith(('.jpg', '.jpeg', '.png')):  # Regola per i tuoi formati di immagine
            ...
            if etichetta_previsione == etichetta_reale:
                previsioni_corrette += 1

    # Stampa i risultati
    print(f"\nTotale Immagini: {totale_immagini}")
    print(f"Previsioni Corrette: {previsioni_corrette}")
    print(f"Accuratezza: {previsioni_corrette / totale_immagini * 100:.2f}%")
